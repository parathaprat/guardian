...
was the true severity, why — that the agent never sees. It is consulted in exactly
...
`static` (fixed rules, no memory), `cold` (agent, empty memory), `learned` (agent,
trained memory) — and reports the delta. Same events, same hidden truth, same
...
that the oracle is a proxy for a human operator, not a human operator. If a founder
is going to trust a number, they should trust it *because* the caveats are stated.
